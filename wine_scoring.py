...
class WineScorer:
    ...
    def calculate_scores(
        ...
        Args:
            semantic_results: Liste de tuples (vin, score_sémantique)
            user_query: Requête utilisateur complète
            dish_info: Informations sur le plat extraites de la requête
            preferred_type: Type de vin préféré détecté dans la description
            filters: Dictionnaire des filtres (budget, région, etc.)
            intensite_aromatique: Niveau d'intensité demandé (1-4)
            top_n: Nombre de vins à retourner
            
        Returns:
            Liste de tuples (vin, score_final, score_sémantique) triés par score décroissant
        """
        scored_wines = []
        ...
        for wine, semantic_score in semantic_results:
            # Seuil minimum de similarité (augmenté pour éviter résultats hors contexte)
            if semantic_score < 0.4:
                continue
            
            # Filtrer par préférence de type si détectée
            if preferred_type:
            ...
            # Vérifier les négations (ex: "ce n'est pas un vin d'apéro")
            final_score = self._check_negations(wine_full_text, query_lower, final_score)
            
            # Filtrage par type selon le plat recherché
            ...
            # Filtrage spécifique pour apéro
            ...
            # Filtrage spécifique pour fromage
    ...
    def _should_exclude_for_red_meat(self, wine: Dict, query_lower_type: str) -> bool:
        ...
        # Exclure rosés et blancs pour viande rouge
        if 'rosé' in wine_type_lower or 'rose' in wine_type_lower or 'blanc' in wine_type_lower:
            return True
        
        # Exclure vins uniquement pour veau (viande blanche)
    ...
    def _should_exclude_for_apero(self, wine: Dict, query_lower_type: str, preferred_type: Optional[str]) -> bool:
        ...
        # Si préférence pour blanc/rosé, exclure tous les rouges
        if preferred_type and preferred_type.lower() != 'rouge':
            if 'rouge' in wine_type_lower:
                return True
        # Sinon, exclure seulement les rouges corsés
        ...
        # Exclure si recherche frais/léger et vin corsé
        ...
        # Exclure si vin dit explicitement "pas pour apéro"
