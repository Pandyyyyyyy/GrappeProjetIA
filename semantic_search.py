...
class SemanticWineSearch:
    ...
    def __init__(self, model_name: str = "paraphrase-multilingual-MiniLM-L12-v2"):
        ...
        Args:
            model_name: Nom du modèle SBERT à utiliser
                       Options: paraphrase-multilingual-MiniLM-L12-v2 (multilingue, rapide)
                                ou paraphrase-multilingual-mpnet-base-v2 (plus précis mais plus lent)
        """
    ...
    def compute_embeddings(self, wines: List[Dict], force_recompute: bool = False):
        ...
        Args:
            wines: Liste des dictionnaires de vins (avec description_fusionnee déjà créée)
            force_recompute: Si True, recalcule même si un fichier existe
        """
        self.wines = wines
    ...
    def search_similar(self, query: str, top_k: int = 10) -> List[Tuple[Dict, float]]:
        ...
        Returns:
            Liste de tuples (vin, score_de_similarité) triés par score décroissant
        """
    ...
    def search_similar_in_wines(
        ...
        Args:
            query: Requête textuelle de l'utilisateur
            wines_subset: Sous-ensemble de vins à rechercher (déjà filtrés)
            top_k: Nombre de résultats à retourner
